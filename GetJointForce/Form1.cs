            ...
            if (BT_Start.Text == "开始")
            {
                ...
                Task.Factory.StartNew(() =>
                {
                    ...
                    for(int i=0;i<6;i++)
                    {
                        ...
                        byte[] clr = new byte[3];
                        rd.NextBytes(clr);
                        ls[i] = new LineSeries("关节" + (i + 1).ToString());
                        ls[i].Color = OxyColor.FromRgb(clr[0], clr[1], clr[2]);
                        pm.Series.Add(ls[i]);
                        Thread.Sleep(10);
                    }
                        ...
                        if (ReadCount > ChartCount)
                        {
                            ...
                            for (int i = ChartCount; i < ReadCount; i++)
                            {
                                ...
                                for(int j=0;j<6;j++)
                                {
                                    ...
                                    ls[j].Points.Add(new DataPoint(speeddata[i % 100, 0], speeddata[i % 100, 1+j]));
                                }
